...
impl Message {
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        // Message tag, prefix message length
        let mut message = Vec::with_capacity(1 + 4 + self.payload.len());
    ...
    pub fn from(value: &[u8]) -> Option<(Self, usize)> {
        ...
        let mut lenght_bytes = [0u8; 4];
        lenght_bytes.copy_from_slice(&value[..4]);
        let length = u32::from_be_bytes(lenght_bytes) as usize;
    ...
    pub fn read_message(stream: &mut TcpStream) -> Self {
        ...
        eprintln!("first buffer byte: {}", buf[0]);
